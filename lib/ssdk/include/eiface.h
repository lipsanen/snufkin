...
public:
	...
	virtual bool CheckBoxInPVS(const Vector& mins,
	                           const Vector& maxs,
	...
	virtual void EmitAmbientSound(int entindex,
	                              const Vector& pos,
	                              const char* samp,
	                              float vol,
	                              int soundlevel,
	...
	virtual void FadeClientVolume(const edict_t* pEdict,
	                              float fadePercent,
	...
	virtual int SentenceGroupPickSequential(int groupIndex,
	                                        char* name,
	                                        int nameBufLen,
	                                        int sentenceIndex,
	                                        int reset) = 0;
	...
	virtual void StaticDecal(const Vector& originInEntitySpace,
	                         int decalIndex,
	                         int entityIndex,
	                         int modelIndex,
	                         bool lowpriority) = 0;
	...
	virtual void* EntityMessageBegin(int ent_index, ServerClass* ent_class, bool reliable) = 0;
	// Begin a usermessage from the server to the client .dll
	virtual void* UserMessageBegin(IRecipientFilter* filter, int msg_type) = 0;
	...
	virtual void PlaybackTempEntity(IRecipientFilter& filter,
	                                float delay,
	                                const void* pSender,
	                                const SendTable* pST,
	                                int classID) = 0;
	...
	virtual void SolidMoved(edict_t* pSolidEnt,
...
public:
	...
	virtual void GetSaveComment(char* comment,
	                            int maxlength,
	                            float flMinutes,
	                            float flSeconds,
...
public:
	...
	// with ehandles.
	virtual void CheckTransmit(void* pInfo, const unsigned short* pEdictIndices, int nEdicts) = 0;
};
...
public:
	...
	virtual bool ClientConnect(edict_t * pEntity,
	                           const char* pszName,
	...
	virtual void* GetPlayerState(edict_t * player) = 0;
...
public:
	virtual bool CreateMessage(const char* plugin, edict_t* pEntity, int type, void* data) = 0;
};
