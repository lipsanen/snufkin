...
#define DECL_MEMBER_STDCALL(retType, name, ...) \
	using _##name = retType(__stdcall*)(__VA_ARGS__); \
	_##name ORIG_##name = nullptr;

#define DECL_HOOK_STDCALL(retType, name, ...) \
	DECL_MEMBER_STDCALL(retType, name, ##__VA_ARGS__) \
	static retType __stdcall HOOKED_##name(__VA_ARGS__)

#define IMPL_HOOK_STDCALL(owner, retType, name, ...) retType __stdcall owner::HOOKED_##name(__VA_ARGS__)
...
public:
	...
	static void LoadFeatures(patterns::PatternCache* cache);
...
private:
	static void InitModules(patterns::PatternCache* cacher);
	static void Hook();
	static void Unhook();
};
...
public:
	virtual Feature* CreateNewInstance() { return new T(); }

	virtual void Move(Feature* instance) { *((T*)this) = std::move(*(T*)instance); }
};
