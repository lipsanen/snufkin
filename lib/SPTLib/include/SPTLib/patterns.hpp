...
namespace patterns {
	...
	struct Pattern {
		...
		constexpr Pattern(const char* pattern) : bytes(), mask() {
			origPattern = pattern;
...
#define PATTERN(pattern) ::patterns::Pattern<::patterns::count_bytes(pattern)>(pattern)
	...
	public:
		...
		constexpr PatternWrapper(const char* name, const Pattern<PatternLength>& pattern)
		    : name_(name), bytes(pattern.bytes), mask(pattern.mask), length_(PatternLength), origPattern(pattern.origPattern) {}

		template<size_t PatternLength>
		constexpr PatternWrapper(const Pattern<PatternLength>& pattern) : PatternWrapper("", pattern) {}
...
#define PATTERNS(name, ...) \
	...
	constexpr auto name = ::patterns::make_pattern_array(NAME_PATTERNS(name, __VA_ARGS__));
} // namespace patterns
