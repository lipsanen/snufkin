	...
	void AttachDetours(const wstring& moduleName, size_t n, const pair<void**, void*> funcPairs[]) {
		...
		for (size_t i = 0; i < n; ++i) {
			...
			if (*target && detour) {
				...
				if (status != MH_OK) {
					EngineWarning("Error hooking %s!0x%p => 0x%p: %s.\n",
				...
				if (status != MH_OK) {
					EngineWarning("Error queueing a hook %s!0x%p => 0x%p for enabling: %s.\n",
					              Convert(moduleName).c_str(),
					              original,
					              detour,
					              MH_StatusToString(status));
		...
		if (hook_count == 0) {
			EngineDevMsg("No %s functions to hook.\n", Convert(moduleName).c_str());
			return;
		}

		auto status = MH_ApplyQueued();
		if (status == MH_OK) {
			EngineDevMsg("Hooked %u %s function%s.\n",
			             static_cast<unsigned int>(hook_count),
			             Convert(moduleName).c_str(),
			             hook_count > 1 ? "s" : "");
		} else {
			EngineWarning("Error applying %u queued hooks for %s: %s.\n",
			              static_cast<unsigned int>(hook_count),
			              Convert(moduleName).c_str(),
			              MH_StatusToString(status));
		}
	...
	void DetachDetours(const wstring& moduleName, size_t n, void** const functions[]) {
		...
		for (size_t i = 0; i < n; ++i) {
			...
			if (*tramp) {
				...
				if (status != MH_OK) {
					EngineWarning("Error unhooking %s!0x%p => 0x%p: %s.\n",
					              Convert(moduleName).c_str(),
					              original,
					              *tramp,
					              MH_StatusToString(status));
		...
		if (hook_count) {
			EngineDevMsg("Unhooked %u %s function%s.\n",
			             static_cast<unsigned int>(hook_count),
			             Convert(moduleName).c_str(),
			             hook_count > 1 ? "s" : "");
		} else {
			EngineDevMsg("No %s functions to unhook.\n", Convert(moduleName).c_str());
			return;
		}
	}
} // namespace DetoursUtils
